    ...
    <div className="max-w-2xl p-6 mx-auto mt-10">
      ...
      <div className="flex items-center mb-4">
        <input
          ...
          className="mr-2"
        />
        <label htmlFor="hideCompleted" className="text-gray-700 dark:text-gray-300">
          የተጠናቀቁትን ደብቅ
        </label>
      </div>
      {filteredTasks.length === 0 ? (
        ...
        <ul className="space-y-6">
          {filteredTasks.map((task, index) => (
            ...
            >
              <div>
                <h3 className={`mb-2 text-xl font-bold text-gray-900 dark:text-white ${task.status === 'completed' ? 'line-through' : ''}`}>{task.title}</h3>
                <p className={`text-gray-600 dark:text-gray-300 ${task.status === 'completed' ? 'line-through' : ''}`}>
              ...
              <div className="space-x-4">
                {task.status !== 'completed' && (
                  <button
                    ...
                    className="px-4 py-2 text-white transition duration-300 bg-green-600 rounded-lg hover:bg-green-700 dark:bg-green-500 dark:hover:bg-green-600 glow-on-hover"
                  >
                    ጨርስ
                  </button>
                )}
